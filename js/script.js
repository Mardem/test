var qtdeFuncionarios, qtdeProLabores, qtdeDocsContabeis, qtdeDocsFiscais, minPrice, maxPrice, maxDocsFiscais,
    maxDocsContabeis, maxProLabores, planos, sn_lp;
$(function () {
    $.get('http://localhost:3000/data').done(function (data, textStatus, jqXHR) {
        planos = data.planos;
...
function simulateMonthlyPayment(qtdeFuncionarios, qtdeDocFiscais) {
    ...
    for (var i in planos) {
        if (qtdeDocFiscais <= parseInt(planos[i].total_documento_fiscal) && parseFloat(planos[i].valor) < minPrice) {
